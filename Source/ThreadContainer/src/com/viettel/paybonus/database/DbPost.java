...
import com.viettel.paybonus.obj.SubscriberPost;
...
public class DbPost {
    ...
    public ArrayList<String> getAllVasList(String subId) {
        ...
        try {
            ...
            logTimeDb("Time to select CM_POS2.SUB_REL_PRODUCT", timeStart);
            ...
            try {
                ...
                logger.error("ERROR retry select CM_POS2.SUB_REL_PRODUCT", ex1);
            }
        } catch (Exception ex) {
            logger.error("ERROR select CM_POS2.SUB_REL_PRODUCT", ex);
        }
    ...
    public SubscriberPost getSubInfor(String isdn) {
        SubscriberPost subscriberPost = null;
        ParamList paramList = new ParamList();
        try {
            ...
            paramList.add(new Param("END_DATETIME", null, Param.DataType.DATE, Param.OUT));
            paramList.add(new Param("STATUS", null, Param.DataType.INT, Param.OUT));
            ...
            paramList.add(new Param("CUST_ID", null, Param.DataType.LONG, Param.OUT));
...
//            paramList.add(new Param("LANGUAGE", null, Param.DataType.STRING, Param.OUT));
            ...
            timeStart = System.currentTimeMillis();
            DataResources data = this.dataStore.selectTable(paramList, "SUB_MB");
            logTimeDb("Time to select CM_POS2.SUB_MB", timeStart);

            subscriberPost = new SubscriberPost();
            if (data.next()) {
                subscriberPost.setSubId(data.getLong("SUB_ID"));
                subscriberPost.setContractId(data.getLong("CONTRACT_ID"));
                subscriberPost.setProductCode(data.getString("PRODUCT_CODE"));
                subscriberPost.setActStatus(data.getString("ACT_STATUS"));
                subscriberPost.setStaDatetime(data.getDate("STA_DATETIME"));
                subscriberPost.setEndDatetime(data.getDate("END_DATETIME"));
                subscriberPost.setStatus(data.getInt("STATUS"));
                subscriberPost.setSerial(data.getString("SERIAL"));
                subscriberPost.setImsi(data.getString("IMSI"));
                subscriberPost.setCustId(data.getLong("CUST_ID"));
                subscriberPost.setNumResetZone(data.getInt("NUM_RESET_ZONE"));
                subscriberPost.setNumResetZone(data.getInt("NUM_RESET_ZONE"));
//                String language = (data.getString("LANGUAGE") == null) ? Commons.defaultLang : data.getString("LANGUAGE").toLowerCase() ;
//                subscriberPost.setLanguage(language);
            }
        } catch (SQLException ex) {
            logger.error("ERROR select CM_POS2.SUB_MB", ex);
            try {
                timeStart = System.currentTimeMillis();
                DataResources data = this.dataStore.selectTable(paramList, "SUB_MB");
                logTimeDb("Time to select CM_POS2.SUB_MB", timeStart);
                ...
                if (data.next()) {
                    ...
                    subscriberPost.setCustId(data.getLong("CUST_ID"));
                    subscriberPost.setNumResetZone(data.getInt("NUM_RESET_ZONE"));
//                    subscriberPost.setLanguage(data.getString("LANGUAGE"));
//                    String language = (data.getString("LANGUAGE") == null) ? Commons.defaultLang : data.getString("LANGUAGE").toLowerCase() ;
//                    subscriberPost.setLanguage(language);
                }
            } catch (Exception ex1) {
                logger.error("ERROR retry select CM_POS2.SUB_MB", ex1);
            }
        } catch (Exception ex) {
            logger.error("ERROR select CM_POS2.SUB_MB", ex);
        }
...
//            paramList.add(new Param("LANGUAGE", null, Param.DataType.STRING, Param.OUT));
            ...
            if (rs.next()) {
                ...
                activeTime = (rs.getDate("STA_DATETIME") != null)
                        ? formatDate.format(rs.getDate("STA_DATETIME")) : "";
...
//                    language = rs.getString("LANGUAGE") == null ?  Commons.defaultLang : rs.getString("LANGUAGE").toLowerCase();
    ...
    /**
     * Lấy contract_offer_id của thuê bao trả sau
     *
     * @param productCode
     * @return
     */
    public long getContractOfferId(String isdn) {
//        String sql = "select co.offer_id from cm_pos.contract_offer co, cm_pos.sub_mb s "
//                + "where co.contract_id = s.contract_id and co.group_id is null and co.status=1 "
//                + "and s.isdn = ? and s.status = ?";
        String sql = "select co.offer_id, s.isdn "
                + "from " + dataStore.getSchema() + ".contract_offer co, "
                + "" + dataStore.getSchema() + ".sub_mb s, " + dataStore.getSchema() + ".offer_sub os "
                + "where co.contract_id = s.contract_id and group_id is null "
                + "and s.status = 2 and os.sub_id = s.sub_id and os.contract_offer_id = co.contract_offer_id "
                + "and os.status = 1 and co.status = 1 and s.isdn = ?";
        ParamList paramList = new ParamList();
        long offerId = -1;
        try {
            paramList.add(new Param("OFFER_ID", null, Param.DataType.LONG, Param.OUT));
...
//            paramList.add(new Param("STATUS", 2, Param.DataType.INT, Param.IN));
            ...
            timeStart = System.currentTimeMillis();
            DataResources rs = dataStore.select(paramList, sql);
            logTimeDb("Time to getContractOfferId", timeStart);
            offerId = 0;
            if (rs.next()) {
                offerId = rs.getLong("OFFER_ID");
            }
        } catch (SQLException ex) {
            logger.error("ERROR getContractOfferId", ex);
            offerId = -1;
            try {
                timeStart = System.currentTimeMillis();
                DataResources rs = dataStore.select(paramList, sql);
                ...
                logger.error("ERROR retry getContractOfferId", ex1);
            }
        } catch (Exception ex) {
            logger.error("ERROR getContractOfferId", ex);
            offerId = -1;
...
//            AppManager.logTimeDb("Time to insert VAS_REGISTER", this.timeStart, this.logger, this.loggerLabel);
            logTimeDb("Time to updateSubRemove", timeStart);
...
//                AppManager.logTimeDb("Time to insert VAS_REGISTER", this.timeStart, this.logger, this.loggerLabel);
                ...
                this.logger.error("ERROR retry insert CONFIRM", ex1);
            }
        } catch (Exception ex) {
            this.logger.error("ERROR insert CONFIRM", ex);
        }
